...
seed_service() {
  ...
  print_msg "$blue" "Seeding $name..."
  pushd "$path" >/dev/null
  DATABASE_URL="$db_url" npx tsx ./prisma/seed.ts
  popd >/dev/null
  print_msg "$green" "✓ Seeded $name"
}

main() {
  print_msg "$blue" "=== MovieHub | Seed all services ==="
  ...
  # Order matters for cross-service references
  seed_service "movie-service" "$movie_db" "apps/movie-service"
  seed_service "cinema-service" "$cinema_db" "apps/cinema-service"
  seed_service "booking-service" "$booking_db" "apps/booking-service"
  seed_service "user-service" "$user_db" "apps/user-service"

  print_msg "$green" "All seeds completed."
}
